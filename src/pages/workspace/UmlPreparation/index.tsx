    ...
    <div className="max-w-6xl mx-auto flex flex-col h-full pb-24">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8 shrink-0">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3">
          ...
          <style>{`
            @keyframes uml-preparation-popover-in {
              from { opacity: 0; transform: translateY(-4px) scale(0.98); }
              to { opacity: 1; transform: translateY(0) scale(1); }
            }
          `}</style>
          <button onClick={() => saveUmlPreparation(umlPreparation)} disabled={saveStatus === "saving" || isAiBusy} className="px-4 py-2 rounded-md bg-primary text-on-primary text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50">
            Save now
          </button>
          ...
          <div className="relative" ref={refinePopoverRef}>
            ...
            {refineOpen && (
              <div
                ...
                style={{ animation: "uml-preparation-popover-in 150ms ease-out" }}
              >
                <textarea
